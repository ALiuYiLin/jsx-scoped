...
- **会话结束请清理**：registry 持有全部会话状态（文件归属 / HMR 失效映射 /
  内联登记 / 提示去重），默认不自动清空。Vite 插件只会在 `isolated: true`
  （本插件独占 registry）时，于 dev server `close` 与单次 build
  （`closeBundle`，watch 模式除外）后自动 `dispose()`；**默认进程级单例可能
  与其它实例共享（如 vitepress-react 核心 md 管线），单个插件绝不可擅自清空**
  ——否则会抹掉其它实例刚 transform 登记的内联样式，导致后续 load 扑空。
  显式传入 `options.registry` 时生命周期归调用方，插件不自动清理。编程式/测试
  场景在 teardown 手动调用 `pipeline.registry.reset()`（`dispose()` 是语义别名）。
  清空后旧虚拟 id 无法 load（得到友好错误）；Map/Set 引用保留，实例可继续复用。
  注意：仅在没有其它活跃会话共享同一 registry 时清理；
